...
describe('InflationSetting', () => {
  ...
  it('takes a rate saved from elsewhere over its own draft', () => {
    const { rerender } = render(<InflationSetting value={0.02} onChange={vi.fn()} />)
    rerender(<InflationSetting value={0.04} onChange={vi.fn()} />)
    expect(input()).toHaveValue('4,0')
  })

  it('only shows the rate when nothing can be written', () => {
    render(<InflationSetting value={0.03} />)
