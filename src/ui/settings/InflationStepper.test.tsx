...
describe('InflationStepper', () => {
  ...
  it('shows the saved rate and saves a typed one as a setting', () => {
    ...
    render(<InflationStepper settings={settings(0.02)} onChange={onChange} />)
  ...
  it('does not step back to an earlier value while a newer one waits to be sent', async () => {
    ...
    function Parent() {
      ...
      return <InflationStepper settings={settings(saved)} onChange={onChange} />
    }
    render(<Parent />)
    const up = () => fireEvent.click(screen.getByRole('button', { name: 'Increase percentage' }))
  ...
  it('takes a rate saved from elsewhere over its own draft', () => {
    const { rerender } = render(<InflationStepper settings={settings(0.02)} onChange={vi.fn()} />)
    rerender(<InflationStepper settings={settings(0.04)} onChange={vi.fn()} />)
