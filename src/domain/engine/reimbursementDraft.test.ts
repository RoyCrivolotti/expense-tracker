...
describe('buildReimbursementDraft', () => {
  ...
  it('opens on the full claim, since covering all of it is the common case', () => {
    const draft = buildReimbursementDraft(group([txn(1), txn(2)]), [account(1)], 1)

    expect(draft?.amountCents).toBe(20_000)
  })

  it('names the claim it settles', () => {
    expect(buildReimbursementDraft(group([txn(1)]), [account(1)], 1)?.description).toBe(
  ...
  it('refuses a claim with nothing outstanding', () => {
    ...
    expect(buildReimbursementDraft(settled, [account(1)], 1)).toBeNull()
  })
})

describe('buildReimbursementDraft — what can be selected', () => {
...
describe('reimbursementAccountId', () => {
  it('never picks a deferred card, even when it is the default', () => {
    ...
    expect(reimbursementAccountId([CARD, account(2)], 9)).toBe(2)
  })

  it('honours the default account when it settles immediately', () => {
    expect(reimbursementAccountId([account(1), account(2)], 2)).toBe(2)
  })

  it('skips archived accounts', () => {
    expect(reimbursementAccountId([account(1, { active: false }), account(2)], 1)).toBe(2)
  })
  ...
  it('falls back to any active account when none settle immediately', () => {
    ...
    expect(reimbursementAccountId([CARD], 9)).toBe(9)
  })
})
