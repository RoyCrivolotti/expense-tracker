...
describe('yearsToAmount', () => {
  it('is zero for an amount already held, and interpolates inside the crossing year', () => {
    ...
    const points = projectNetWorth(scenarioToParams(plan, DEFAULT_INFLATION_RATE))
...
describe('milestoneCrossingDate', () => {
  it('places the crossing on the calendar from the plan start', () => {
    const points = projectNetWorth(scenarioToParams(plan, DEFAULT_INFLATION_RATE))
    expect(milestoneCrossingDate(plan, points[2]!.investedCents, DEFAULT_INFLATION_RATE)).toBe('2028-01-01')
    expect(milestoneCrossingDate(plan, points[3]!.investedCents, DEFAULT_INFLATION_RATE)).toMatch(/^2029-01-0[12]$/)
  })
...
describe('milestoneStanding', () => {
  ...
  it('gives the expected date alone without a target, and says so past the horizon', () => {
    expect(milestoneStanding(inYearTwo, plan, undefined, DEFAULT_INFLATION_RATE)).toEqual({ kind: 'expected', expected: '2028-01-01' })
