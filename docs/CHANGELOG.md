...
- **A chart's tooltip opens on the side of the chart with more room**, above or below it, and stays there while you slide along the chart. It is laid over the page instead of sitting in it, so it takes no space, the chart never moves and nothing scrolls to reach it. Every chart with a tooltip does this: the projection charts (the main one had none, its values were only in a legend below it), the income and expense charts and the category pie. Wide screens keep the tooltip at the pointer. When the room is below the chart, the tooltip is pinned to the screen just above the tab bar instead of hanging under the chart, so it is whole and stays put while you scroll. A tooltip too tall for the room on either side, such as the purchase breakdown on a small phone, then covers the chart's lower edge, or loses its first rows above, whichever costs less.
- **The tooltip stays with its chart as you scroll.** It sits against the chart's edge and moves with it instead of following the screen, so it is not left over the next card once the chart has gone. It moves to the screen when the page scrolls and there is no longer room for it above the chart, and it is not shown while its chart is mostly off screen. Scrolling does not close it, since the breakdown may sit below the fold; a tap outside does.
- **The main projection chart shows one readout at a time.** With its legend fully on screen the legend is the readout and there is no tooltip. Otherwise the tooltip is, and the legend's figures step aside (their space is kept, so nothing moves). With the chart scrolled away and only part of the legend on screen, the legend shows the figures again.
