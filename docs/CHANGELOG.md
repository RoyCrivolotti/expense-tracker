...
- **Installment rows are easier to read.** Each row used to run its whole story together in one dotted line. It now has the name and amount on top, the state and position underneath ("Due 12 Jun · 2 of 24"), and a Debit or Credit chip with the plan's last payment month below that, with one Due, Forecast or Paid pill. A credit-card installment no longer shows Paid and Forecast together, and once its statement is paid it shows the day you recorded paying it instead of the day of the charge. Manage plans uses the same words.
