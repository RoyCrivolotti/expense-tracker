...
There is one rate, and it is the owner's: `settings.assumed_inflation` (`ExpenseSettings.assumedInflation`), read as 2% (`DEFAULT_INFLATION_RATE`, about the ECB target) until it is set, and held between 0 and 10% (`INFLATION_MIN` and `INFLATION_MAX`, checked by the one `assumedInflationError` the API, the in-memory double and the control share). It is set in Setup, beside the cash reserve target and the other assumptions Progress is measured with, and can be stepped under the Plan chart, beside the Nominal / Purchasing power toggle, to watch the plan move. Both are the same `InflationStepper` on the one saved value, so a step on the chart is a saved change that every Goals view reads, not a preview; `useSavedInflation` keeps the newest step while a save is in flight and puts the saved value back if it fails. It is a setting of the owner rather than of a scenario so that every scenario, and every row of the comparison table, is in the same money.
