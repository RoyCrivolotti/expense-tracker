...
The hero chart shows the plan in today's money by default. Its footer toggle switches to a nominal view, which inflates the plan line and its band at a rate the user can adjust there (the same 2% by default), and leaves the check-in dots as they are, since they are already nominal:
...
The stepper's rate belongs to the nominal view alone. Every comparison with actuals, meaning the on/off-track line, "Where you are today", the dots in today's money, "Actual vs plan" and the measured return, uses `DEFAULT_INFLATION_RATE`, so moving the stepper cannot make the chart disagree with the status written beside it. The FI target and the milestones are targets in today's money and a reference line is flat, so the nominal view does not draw them, and says so beside the toggle. On Progress, "Actual vs plan" says it is in today's money, and "Net worth over time" says its balances are as logged, in the money of each day.
